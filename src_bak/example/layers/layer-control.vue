  ...
  >
    <template #bodyCell="{ column, record }">
      <template v-if="column.dataIndex === 'edit'">
        <a-space>
          <mars-icon icon="aiming" color="#f2f2f2" class="icon-vertical-a" title="调整图层" @click.stop="modifyLayer(record)" />
...
onMounted(() => {
  ...
  for (let i = layers.length - 1; i >= 0; i--) {
    ...
    if (layer.isPrivate || layer.options.layerType === "base") {
      continue
    }
  ...
  map.on(mars3d.EventType.addLayer, function (event) {
    ...
    if (layer.isPrivate || layer.name === "POI查询" || layer.options.layerType === "base") {
      return
    }
...
// 表格行：点击定位图层
const modifyLayer = (record: LayerTableItem) => {
  const layer = map.getLayerById(record.key)
  if (layer) {
     formState.layerName = record.name
  }
}
